...
import quoteStackImg from '../assets/quote-stack.jpeg';
    ...
    <div className="quote-stack">
      {props.map(({ x, y, rot, scale }, i) => (
        <animated.div className="quote-stack__card-container" key={i} style={{ transform: interpolate([x, y], (x, y) => `translate3d(${x}px,${y}px,0)`) }}>
          ...
          <animated.div className="quote-stack__card" {...bind(i)} style={{ transform: interpolate([rot, scale], trans), backgroundImage: `url(${cards[i]})`, touchAction: 'none' }} />
        </animated.div>
      ))}
    </div>
  );
